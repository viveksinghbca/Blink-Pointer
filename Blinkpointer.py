...
def calibrate_blink():
    ...
    for _ in range(20):
        ...
        if results.multi_face_landmarks:
            ...
            open_eye_distances.append((get_eye_distance(lm, 159, 145) + get_eye_distance(lm, 386, 374)) / 2)
        cv2.waitKey(30)
    ...
    for _ in range(20):
        ...
        if results.multi_face_landmarks:
            lm = results.multi_face_landmarks[0].landmark
            closed_eye_distances.append((get_eye_distance(lm, 159, 145) + get_eye_distance(lm, 386, 374)) / 2)
        cv2.waitKey(30)
...
tk.Label(frame, text="Eye Mouse - Enhanced", font=("Arial", 16, "bold"), bg="#f0f8ff", fg="#00796b").pack(pady=10)

start_btn = tk.Button(frame, text="Start Tracking", command=start_tracking, **button_style)
start_btn.pack(pady=5)

stop_btn = tk.Button(frame, text="Stop Tracking", command=stop_tracking, state="disabled", **button_style)
stop_btn.pack(pady=5)

calibrate_btn = tk.Button(frame, text="Calibrate Blink", command=calibrate_blink, **button_style)
calibrate_btn.pack(pady=5)

load_btn = tk.Button(frame, text="Load Calibration", command=load_calibration, **button_style)
load_btn.pack(pady=5)

overlay_btn = tk.Button(frame, text="Overlay: ON", command=toggle_overlay, **button_style)
overlay_btn.pack(pady=5)

tk.Label(frame, text="Double Blink App", font=("Arial", 11, "bold"), bg="#f0f8ff", fg="#004d40").pack(pady=(10, 0))

selected_app = tk.StringVar()
app_menu = ttk.Combobox(frame, textvariable=selected_app, width=30)
...
exit_btn = tk.Button(frame, text="Exit", command=root.destroy, **button_style)
